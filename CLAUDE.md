...
> `ruff check` + `ruff format --check`, `mypy app/services app/schemas`, `eslint --max-warnings 0` and `prettier --check`; `pytest`; `vitest`, an API-type freshness check
...
- **A migration altering an existing table uses** `batch_alter_table(..., naming_convention=NAMING)` with the convention from `0020_past_papers.py`, and names new
...
from `backend/`, `python -c "import json;from app.main import app;print(json.dumps(app.openapi(),indent=2))" > ../frontend/openapi.json`, then
...
after finishing a task spawn subagents  and divide the tasks into smaller parts each subagent job is to check that their assigned part of the code works, spawn up subagents from ECC or anyother plugin 

code reviewer

always manually  trigger coderabbit reviews 

sustanibailty/maintaibiality 

if any changes happen that affect docs then edit the docs never ever have docs outdated for example if a change  with something releat to deployement happens or storage change all docs which would happen to be outdated 