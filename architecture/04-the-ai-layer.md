...
| **Class brief** | A pre-lesson summary for the tutor, built live and discarded | Claude Opus |
| **Narrative** | The stored plain-prose paragraph a class or parent screen opens with | Claude Opus |

Each of these is a *routing* key — which provider and model answers. For cost reporting they collapse into five billing buckets, because syllabus meters as extraction, and both the class brief and the narrative meter as reports.

The narrative is worth distinguishing from the class brief, since they produce similar-looking text: the narrative is generated by a background job and stored, so the screen never waits on a model to render its main content, while the class brief is built live on request and thrown away.
