...
| **AV-76** | ~~**Marking-context precedence.** The official mark scheme is **absolute and never overridden**.~~ **SUPERSEDED by the owner, 9 Sep 2026 (task 3.2b).** The **tutor's rules beat the official mark scheme**: where they contradict it the marks are awarded the tutor's way and the contradiction is **recorded** (`QuestionMark.scheme_conflict`), for every contradictory point rather than case by case. A mark that overrode the scheme **still auto-finalizes** — `AV-25`'s gate (an attached scheme *and* confident output) is unchanged, and nothing a tutor writes moves it. Resolution order is now: chapter notes → subject rules → mark scheme → exam board and level. |
...
| **AV-94** | ~~**The official mark scheme always wins.**~~ **SUPERSEDED by the owner, 9 Sep 2026 (task 3.2b)** — see `AV-76`. Tutor-authored chapter notes and subject rules **do** override a scheme, and the departure is recorded rather than suppressed. The threat-review criteria this entry carried are otherwise unchanged and were implemented: tutor free text is labelled structurally as data, its length is capped (4,000 chars for chapter notes, 8,000 for subject rules), and the departure never reaches the student — it is on the tutor-gated `MarkRow`, never `StudentMarkRow`, and the prompt forbids putting it in `feedback`. **Known consequence, accepted:** a mark awarded against the scheme does not reflect the real exam, and marks feed `Evidence` → readiness → the predicted grade with nothing downstream correcting for it. The recorded contradiction is the only signal. |
...
| **0.9** | Token-revocation regression test — ✅ done | S | ✅ |
| **0.10** | Fill in the AI price table — ✅ done | S | ✅ |
