...
The `# noqa: BLE001` comments across five modules — `workers/jobs.py`, `main.py`,
`services/timezones.py`, `services/readiness_v2_ai.py`, and `services/storage.py` — are no
longer vestigial: `BLE` is selected, so they suppress a rule that actually runs, and the next
blind `except Exception` is a decision on the record.
