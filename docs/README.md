...
| [`docs/avora-new-state-august-16.md`](avora-new-state-august-16.md) | The **engineering plan** taking Avora from what is built to a ready-to-sell product — 122 product decisions, 26 engineering decisions, and 80 tasks across 14 phases in §7. The spec of record for every phase | Yes, until Phase 12 lands |
| [`docs/avora-plan-handoff.md`](avora-plan-handoff.md) | **Written for agents, not humans.** Where the **whole plan** stands: which of the 14 phases are complete, the next task, the deployment posture that must not change before 11.2, the ADRs still owed, and the risks whose real state differs from the plan's own §10 | Yes, until Phase 12 lands |
| [`docs/av-82-phase-1-handoff.md`](av-82-phase-1-handoff.md) | **Written for agents, not humans.** Phase 1 (`AV-82`) as delivered: what landed in 1.1–1.5, the invariants that must not regress, and the test-infrastructure constraints any future concurrency work will hit. Distinguishes capability built from deployment unchanged (`AV-85`) | Yes, as the Phase 1 record |
| [`docs/av-9-phase-2-handoff.md`](av-9-phase-2-handoff.md) | **Written for agents, not humans.** Phase 2 as delivered: tutor-owned subjects, the `Chapter` table, chapter-first syllabus extraction, one grade-boundary source, teaching guidance and marking rules. Names the three things it stores that nothing reads yet, and which phase owns each | Yes, as the Phase 2 record |
| [`docs/av-81-phase-3-handoff.md`](av-81-phase-3-handoff.md) | **Written for agents, not humans.** Phase 3 as delivered: chapter-scoped classifieds, the marking-context assembler, `AV-76`'s **reversed** precedence, the Gemini retirement, summarised subject rules and typed answers. Says what the three unbuilt tasks will collide with, and what a test can and cannot prove about precedence | Yes, before any Phase 3, 4 or 5 work |
