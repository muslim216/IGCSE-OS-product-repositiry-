...
| [`docs/avora-new-state-august-16.md`](avora-new-state-august-16.md) | The **programme plan** to reach the Avora target state — 104 product decisions recorded verbatim as the source of truth, 26 engineering decisions marked separately, and 13 phases of tasks written for AI agents and engineers to execute one at a time or in parallel. Carries the resolutions of the SWE audit, the security threat review and the engineering audit. **Supersedes parts of `experience-implementation-plan.md`; task 0.0 reconciles the two** | Yes, until Phase 11 lands |
