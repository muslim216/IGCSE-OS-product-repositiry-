...
| [`docs/experience-implementation-plan.md`](experience-implementation-plan.md) | The **delivery plan** for that spec — every state each surface can be in, the copy it says in each, and the 30 pull requests that build it, in order. **Delivered: PRs 1–29 have shipped.** Kept as the record of why the current code looks as it does; superseded by `avora-new-state-august-16.md` wherever the two disagree | No longer executed; kept for its decisions |
...
| [`docs/avora-new-state-august-16.md`](avora-new-state-august-16.md) | The **programme plan** to reach the Avora target state — 122 product decisions recorded verbatim as the source of truth, 26 engineering decisions marked separately, and 14 phases of tasks written for AI agents and engineers to execute one at a time or in parallel. Settles the navigation for all three roles, and carries the resolutions of the SWE audit, the security threat review and the engineering audit. **Supersedes parts of `experience-implementation-plan.md`; task 0.0 reconciles the two** | Yes, until Phase 12 lands |
