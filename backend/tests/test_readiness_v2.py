...
async def test_evaluate_subject_factors_end_to_end(client, tutor, world):
    ...
    async with async_session() as session:
        ...
        lesson = Lesson(
            organization_id=org_id, group_id=group_id, date=date.today() - timedelta(days=10),
            duration_min=60,
        )
        session.add(lesson)
        await session.flush()
        ...
        session.add(
            Evidence(
                student_id=student_id, topic_id=topic1, source_type=EvidenceSource.observation,
                score_pct=60.0, max_marks=0, occurred_at=NOW - timedelta(days=3),
            )
        )

        # A finalized homework submission with one marked question on topic1.
        assignment = Assignment(
            group_id=group_id, title="HW1", status=AssignmentStatus.published,
            due_at=NOW - timedelta(days=2),
        )
        session.add(assignment)
        await session.flush()
        question = AssignmentQuestion(
            assignment_id=assignment.id, position=0, number="1", text_summary="Q1",
            max_marks=10, has_mark_scheme=True, difficulty=QuestionDifficulty.hard,
        )
        ...
        submission = Submission(
            ...
            status=SubmissionStatus.finalized, submitted_at=NOW - timedelta(days=3),
            finalized_at=NOW - timedelta(days=1),
        )
        session.add(submission)
        await session.flush()
        mark = QuestionMark(
            submission_id=submission.id, question_id=question.id, final_marks=8,
        )
        session.add(mark)
        await session.flush()
        ...
        session.add(
            Mistake(
                student_id=student_id, question_mark_id=mark.id, topic_id=topic1,
                category=MistakeCategory.careless, severity=1,
            )
        )

        # A past paper attempt for the subject.
        past_paper = PastPaper(
            organization_id=org_id, subject_id=subject_id, session_label="June 2026", paper_number="1",
        )
        session.add(past_paper)
        await session.flush()
        session.add(
            PastPaperAttempt(
                past_paper_id=past_paper.id, student_id=student_id, raw_marks=15, max_marks=20,
                timed=True, attempted_at=date.today() - timedelta(days=5),
            )
        )

        # A mock assessment score.
        assessment = Assessment(
            tutor_id=tutor_user.id, subject_id=subject_id, title="Mock", type=AssessmentType.mock,
        ...
        session.add(
            AssessmentScore(
                assessment_id=assessment.id, student_id=student_id, topic_id=topic1,
                marks=14, max_marks=20,
            )
        )
    ...
    async with async_session() as session:
        rows = await evaluate_subject_factors(session, student_id, subject_id, "test-run-1", now=NOW)
        await session.commit()

    by_factor: dict[tuple, FactorEvaluation] = {
        (r.factor, r.topic_id): r for r in rows
    }
